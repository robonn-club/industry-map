...
const statesLayer = L.geoJSON(BUNDESLAENDER_GEO, {
  ...
  onEachFeature: (feat, layer) => {
    ...
    layer.on('click', e => {
      ...
      if (region) { L.DomEvent.stopPropagation(e); openPanel(region); }
    });
  },
}).addTo(map);
...
const bonnCircle = L.circle(BONN, {
  radius: BONN_RADIUS_KM * 1000,   // km → metres
...
// map.createPane('fixedPane');
// map.getPane('fixedPane').style.zIndex = 450;

// const bonnIcon = L.divIcon({
//   className: '',
//   html: `<div class="bonn-pin">
//     <span class="bonn-dot"></span>
//     <span class="bonn-label">Bonn · Robonn</span>
//   </div>`,
//   iconAnchor: [6, 6],
// });

// L.marker(BONN, { icon: bonnIcon, pane: 'fixedPane', interactive: false }).addTo(map);
...
allFilters.forEach(([sector, label, color]) => {
  const count = sector === 'all'
    ? ENTITIES.length
    : ENTITIES.filter(c => c.sector === sector).length;
...
// Single source of truth: a company is shown only if it passes every active
...
function isVisible(c) {
  ...
  const bonnOk   = !bonnFilterActive || c.distKm <= BONN_RADIUS_KM;
  return sectorOk && bonnOk;
}
...
function toggleBonnFilter() {
  ...
  if (bonnFilterActive) {
    ...
    map.flyTo(BONN, 7, { duration: 1 });
  } else {
    map.removeLayer(bonnCircle);
  }
  refreshMarkers();   // out-of-range companies are hidden, in line with the sector filter
  updateStats();
}
...
function openPanel(region) {
  ...
  gridToggleBtn.classList.add('shifted');
}
...
function closePanel() {
  ...
  gridToggleBtn.classList.remove('shifted');
}
...
function buildGrid() {
  ...
  Object.entries(SECTOR_CONFIG).forEach(([sector, cfg]) => {
    const companies = ENTITIES.filter(c => c.sector === sector);
...
document.getElementById('grid-toggle').addEventListener('click', () => toggleGrid(true));
...
const STORAGE_KEY = 'robonn_map_welcome_v1';
...
function dismissWelcome() {
  if (document.getElementById('welcome-skip').checked) {
    localStorage.setItem(STORAGE_KEY, 'dismissed');
  }
  document.getElementById('welcome-overlay').classList.add('welcome-hidden');
}

document.getElementById('welcome-cta').addEventListener('click', dismissWelcome);
document.getElementById('welcome-overlay').addEventListener('click', e => {
  if (e.target.id === 'welcome-overlay') dismissWelcome();
});
...
// Show welcome unless dismissed
if (localStorage.getItem(STORAGE_KEY) !== 'dismissed') showWelcome();
